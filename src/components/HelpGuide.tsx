...
import { HelpCircle, Receipt, Utensils, Users, Calculator, Share2, Wallet, Settings, Moon, StickyNote, ChevronDown, Sparkles, Camera } from "lucide-react";
...
const steps = [
  ...
  },
  {
    ...
    steps: ["Buat judul split bill baru", "Tambahkan nama teman-teman", "Tambahkan item/menu per orang beserta harganya", "Atur pajak & service (dibagi rata ke semua teman)", "Lihat hasil & export ke PDF atau share ke WhatsApp!"],
  },
  {
    icon: Camera,
    title: "Scan Struk (AI)",
    ...
    steps: [
      "Buka tab Split Bill atau Custom Split Bill",
      "Klik tombol 📷 Scan Struk",
      "Pilih 'Ambil Foto' (kamera) atau 'Pilih dari Galeri' (galeri)",
      "Tunggu AI mengenali item & harga dari struk",
      "Review item dan harga, edit jika diperlukan",
      "Di Custom Split: Pilih nama teman untuk setiap item (bisa multi-select)",
      "Klik 'Tambahkan' untuk masukkan ke split bill",
    ],
  },
  {
    icon: StickyNote,
    ...
    steps: ["Klik menu Catatan di halaman utama", "Klik + untuk membuat catatan baru", "Isi judul dan isi catatan, lalu klik Simpan", "Gunakan tombol ✏️ untuk edit dan 🗑️ untuk hapus", "Atur urutan catatan dengan tombol ↑ ↓"],
  },
  {
    icon: Sparkles,
    title: "Patungan AI",
...
const features = [
  ...
  { icon: Camera, label: "Scan Struk pakai AI" },
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-md max-h-[80vh] w-[calc(100%-2rem)] rounded-xl overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Panduan lengkap menggunakan Patungan By Nexteam</DialogDescription>
        </DialogHeader>
            ...
            <motion.div key={section.title} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} className="space-y-2 border border-border rounded-xl overflow-hidden">
              <button onClick={() => setExpandedStep(expandedStep === idx ? null : idx)} className="w-full flex items-center justify-between p-3 bg-muted/30 hover:bg-muted/50 transition-colors">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-semibold text-foreground text-sm text-left">{section.title}</h3>
                </div>
                <motion.div animate={{ rotate: expandedStep === idx ? 0 : -90 }} transition={{ duration: 0.2 }}>
