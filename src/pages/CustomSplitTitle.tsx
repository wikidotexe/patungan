...
const CustomSplitTitle = () => {
  ...
  const deleteBill = (billTitle: string) => {
    if (confirm(`Hapus data untuk "${billTitle}"?`)) {
      ...
      localStorage.removeItem(`custom_patungan_${billTitle}_persons`);
      localStorage.removeItem(`custom_patungan_${billTitle}_enableService`);
      localStorage.removeItem(`custom_patungan_${billTitle}_enableTax`);
      localStorage.removeItem(`custom_patungan_${billTitle}_customService`);
      localStorage.removeItem(`custom_patungan_${billTitle}_customTax`);
                ...
                {savedBills
                  ...
                  .map((bill) => (
                    <div key={bill.title} className="group flex items-center justify-between rounded-xl border border-border bg-card p-4 transition-all hover:shadow-md hover:border-primary/30">
                      ...
                      <button onClick={() => deleteBill(bill.title)} className="h-8 w-8 flex items-center justify-center rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors opacity-0 group-hover:opacity-100">
